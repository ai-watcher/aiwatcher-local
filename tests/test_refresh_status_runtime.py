...
(async () => {
  ...
  assert.equal(classifyUpdateStatus({ok:true,install_kind:'package',update_available:true,can_apply:false}), 'available');
  assert.equal(classifyUpdateStatus({ok:true,install_kind:'source',on_branch:false,update_available:false}), 'current');
  assert.equal(classifyUpdateStatus({ok:true,install_kind:'source',on_branch:false,update_available:true,can_apply:false}), 'available');
