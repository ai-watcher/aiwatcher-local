...
class NativeOverlayConfigTests(unittest.TestCase):
    ...
    def test_presence_bars_draw_the_waiting_queue_and_countdown(self) -> None:
        ...
        self.assertIn("let maxWaitingRows = 5", mac)
        ...
        self.assertNotIn("consoleButton.isHidden = true", mac)
        ...
        self.assertIn("max_waiting_rows = 5", tk_source)
        ...
        self.assertIn('visible_waiting_rows() == 0 or state_var.get() in {"control_review", "context_review"}', tk_source)
    ...
    def test_compaction_rows_ride_the_queue_with_their_own_copy(self) -> None:
        ...
        self.assertIn("func writeClipboard", mac)
        self.assertIn("Copy failed", mac)
        self.assertIn("Paste /compact into that session", mac)
        ...
        self.assertIn("def set_clipboard_text", tk_source)
        self.assertIn("Copy failed", tk_source)
        self.assertIn("Paste /compact into that session", tk_source)
